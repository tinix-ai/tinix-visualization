...
export const option = {
  ...
  visualMap: {
    ...
    orient: 'vertical',
    pieces: [
      { gte: 1000, label: '>1000' }, // Không được chỉ định max,thể hiện max là vô hạn (Infinity）。
      { gte: 600, lte: 999, label: '600-999' },
      { gte: 200, lte: 599, label: '200-599' },
      { gte: 50, lte: 199, label: '49-199' },
      { gte: 10, lte: 49, label: '10-49' },
      { lte: 9, label: '<9' } // Không được chỉ định min,thể hiện min là vô hạn (-Infinity）。
    ],
    inRange: {
      // Độ dốcMàu sắc, từ tuổi thơ đến tuổi trưởng thành
  ...
  series: [
    ...
    },
    {
      ...
      effect: {
        show: true,
        period: 4, //Speed Kim Đồng Hồ，Giá trị càng nhỏ chạy càng nhanh
        trailLength: 0.4, //Độ dài đuôi khói [0, 1] càng lớn đuôi càng dài
        symbol: 'arrow', //biểu tượng mũi tên
