    ...
    <n-space class="go-decorates-more-countdown" :size="flipperGap" align="center" justify="center">
      <template v-if="showDay">
        <flipper
          ...
          class="go-d-block"
        />
        <div v-if="style === window['$t']('packages.auto_418')">{{ $t('phase7.auto_355') }}</div>
        <div v-else>:</div>
      </template>
      <flipper
        :count="item"
        ...
        class="go-d-block"
      />
      <div v-if="style === window['$t']('packages.auto_418')">{{ $t('phase7.auto_10') }}</div>
      <div v-else>:</div>
      <flipper
        :count="item"
        :width="flipperWidth"
        ...
        class="go-d-block"
      />
      <div v-if="style === window['$t']('packages.auto_418')">{{ $t('phase7.auto_434') }}</div>
      <div v-else>:</div>
      <flipper
        :count="item"
        :width="flipperWidth"
        ...
        class="go-d-block"
      />
      <div v-if="style === window['$t']('packages.auto_418')">{{ $t('phase7.auto_141') }}</div>
    </n-space>
  </div>
</template>
...
const getFlipperData = (val: string | number) => {
  ...
  return val
    .toString()
    .padStart(len, '0') // {{ $t('phase7.auto_334') }}Căn chỉnh bên góc phải
    .split('') // Đổi mảng
}
