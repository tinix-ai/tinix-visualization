...
const dragStartHandle = (e: DragEvent, item: ConfigType) => {
  ...
  // Thay đổi mục cấu hình{{ $t('phase7.auto_397') }}để kéo thuộc tính{{ $t('phase7.auto_510') }}
...
// nhấp đúp chuột{{ $t('phase7.auto_481') }}
const dblclickHandle = async (item: ConfigType) => {
  ...
  try {
    ...
    componentInstall(item.chartKey, fetchChartComponent(item))
    componentInstall(item.conKey, fetchConfigComponent(item))
    // {{ $t('phase7.auto_494') }}mớiBiểu đồThành phần
    ...
    loadingError()
    window['$message'].warning(window['$t']('phase7.auto_119'))
  }
}
...
const deleteHandle = (item: ConfigType, index: number) => {
  goDialog({
    message: window['$t']('views_components.auto_3'),
...
/* Danh sáchmục{{ $t('phase7.auto_83') }} */
$itemWidth: 100%;
$maxItemWidth: 180px;
$halfItemWidth: 46%;
/* nội dung{{ $t('phase7.auto_126') }} */
$centerHeight: 100px;
