  ...
  <div class="create-color-setting" v-if="editColor">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true">
      <n-space justify="space-between">
        <!-- Danh xưng -->
        <n-input-group>
          <n-input-group-label>{{ $t('phase7.auto_117') }}</n-input-group-label>
          <n-input
        ...
        <n-tag type="warning">{{ $t('phase7.auto_70') }}</n-tag>
      </n-space>
      <!-- Màu sắc -->
          ...
          <div>
            <n-button type="primary" secondary @click="addColor">
              <div class="go-flex-items-center">
                <span class="go-mr-4">{{ $t('phase7.auto_481') }}</span>
                <n-icon size="16">
                  <add-icon></add-icon>
                </n-icon>
              </div>
    ...
    <div class="expend-color-box">
      <n-card class="go-mt-3 expend-color" :bordered="false" role="dialog" size="small" aria-modal="true">
        <n-text>{{ $t('phase7.auto_183') }}</n-text>
      ...
      <n-card class="go-mt-3 expend-color" :bordered="false" role="dialog" size="small" aria-modal="true">
        <n-text>{{ $t('phase7.auto_457') }}</n-text>
        <n-divider style="margin: 10px 0"></n-divider>
        <n-space :size="[4, 0]" justify="center">
          <div
            class="color-computed-item"
...
const targetColor = reactive<{
  ...
  // -1 có nghĩa là không cóTrạng Trái Hover Activeyếu tố
  index: -1,
  color: ''
})
...
// kết xuất{{ $t('phase7.auto_124') }}màu sắc
